...
//Código do DataTable, não feito pelo professor
     ...
     $sql = "SELECT * FROM PRODUTO WHERE EMPRESA_ID = $EMPRESA_ID AND 1=1 ";//1=1 > Traz tudo
     //SELECT * > Traz todos os dados da tabela (Nesse caso, seria: SELECT ID, NOME, VALOR)
     //WHERE EMPRESA_ID = $EMPRESA_ID > Para filtrar somente os produtos da empresa logada
     ...
     if( !empty( $filtro ) ){//Se esse filtro não estiver vazio (!empty > '!' é negação)
          ...
          $sql .= " OR VALOR LIKE '$filtro%' "; //Por VALOR
     }
